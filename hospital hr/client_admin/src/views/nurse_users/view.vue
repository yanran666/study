		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="护士工号" prop="nurse_no">
												<el-input id="nurse_no" v-model="form_sub['nurse_no']" placeholder="请输入护士工号"
							  v-if="user_group === '管理员' || (form_sub['nurse_users_id'] && $check_field('set','nurse_no')) || (!form_sub['nurse_users_id'] && $check_field('add','nurse_no'))"></el-input>
					<div v-else-if="$check_field('get','nurse_no')">{{form_sub['nurse_no']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="护士姓名" prop="nurse_name">
												<el-input id="nurse_name" v-model="form_sub['nurse_name']" placeholder="请输入护士姓名"
							  v-if="user_group === '管理员' || (form_sub['nurse_users_id'] && $check_field('set','nurse_name')) || (!form_sub['nurse_users_id'] && $check_field('add','nurse_name'))"></el-input>
					<div v-else-if="$check_field('get','nurse_name')">{{form_sub['nurse_name']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="当前科室" prop="current_department">
												<el-input id="current_department" v-model="form_sub['current_department']" placeholder="请输入当前科室"
							  v-if="user_group === '管理员' || (form_sub['nurse_users_id'] && $check_field('set','current_department')) || (!form_sub['nurse_users_id'] && $check_field('add','current_department'))"></el-input>
					<div v-else-if="$check_field('get','current_department')">{{form_sub['current_department']}}</div>
											</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["nurse_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?nurse_users_id=" + form_sub["nurse_users_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/nurse_users/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/nurse_users/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/nurse_users/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/nurse_users/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/nurse_users/view','get');
