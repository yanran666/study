		...
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item label="休假原因">
									<el-select v-model="query.reason_for_vacation">
				                            <el-option v-for="o in list_reason_for_vacation" :key="o" :label="o"
                            	:value="o">
                            </el-option>
										</el-select>
								</el-form-item>
					...
					<el-form-item>
						...
						<el-button v-if="$check_option('/leave_application/table','export_db')" @click="export_db('dataTable')" style="margin-right: 155px;">导出</el-button>
						<router-link v-if="user_group == '管理员' || $check_action('/leave_application/table','add') || $check_action('/leave_application/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/leave_application/table','del') || $check_action('/leave_application/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','starting_and_ending_dates_of_vacation')" min-width="200">
					...
					<el-table-column prop="leave_duration" @sort-change="$sortChange" label="请假时长"
				v-if="user_group == '管理员' || $check_field('get','leave_duration')" min-width="200">
					</el-table-column>
					<el-table-column prop="approval_progress" @sort-change="$sortChange" label="审批进度"
				v-if="user_group == '管理员' || $check_field('get','approval_progress')" min-width="200">
					</el-table-column>
			...
			<el-table-column label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/leave_application/table','set') || $check_action('/leave_application/view','set') || $check_action('/leave_application/view','get') || $check_action('/大科审批/table','add') || $check_action('/大科审批/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/leave_application/table','set') || $check_action('/leave_application/view','set') || $check_action('/leave_application/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
						<!--跨表按钮-->
							<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/dake_approval/view')" v-if="user_group == '管理员' || $check_action('/dake_approval/table','add') || $check_action('/dake_approval/view','add')">
						<span>大科审批</span>
					</el-button>
							</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/leave_application/get_list?like=0",
				url_del: "~/api/leave_application/del?",
		...
		methods: {
			...
			export_db(tableid){
				...
				// this.$get("~/api/leave_application/export_db?",query,(json)=>{
				// 	if(json.result){
