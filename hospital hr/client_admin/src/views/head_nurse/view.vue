			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="状态" prop="state">
					<el-select v-model="form.state" placeholder="请选择">
						...
						<el-col v-if="user_group === '管理员' || $check_field('get','nurse_head_job_number') || $check_field('add','nurse_head_job_number') || $check_field('set','nurse_head_job_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="护士长工号" prop="nurse_head_job_number">
												<el-input id="nurse_head_job_number" v-model="form_sub['nurse_head_job_number']" placeholder="请输入护士长工号"
							  v-if="user_group === '管理员' || (form_sub['head_nurse_id'] && $check_field('set','nurse_head_job_number')) || (!form_sub['head_nurse_id'] && $check_field('add','nurse_head_job_number'))"></el-input>
					<div v-else-if="$check_field('get','nurse_head_job_number')">{{form_sub['nurse_head_job_number']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','nurse_head_name') || $check_field('add','nurse_head_name') || $check_field('set','nurse_head_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="护士长姓名" prop="nurse_head_name">
												<el-input id="nurse_head_name" v-model="form_sub['nurse_head_name']" placeholder="请输入护士长姓名"
							  v-if="user_group === '管理员' || (form_sub['head_nurse_id'] && $check_field('set','nurse_head_name')) || (!form_sub['head_nurse_id'] && $check_field('add','nurse_head_name'))"></el-input>
					<div v-else-if="$check_field('get','nurse_head_name')">{{form_sub['nurse_head_name']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="职位" prop="position">
							  ...
							  v-if="user_group === '管理员' || (form_sub['head_nurse_id'] && $check_field('set','position')) || (!form_sub['head_nurse_id'] && $check_field('add','position'))"></el-input>
	...
	export default {
		...
		data() {
			return {
				...
				disabledObj:{
								"nurse_head_job_number_isDisabled": false,
										"nurse_head_name_isDisabled": false,
										"position_isDisabled": false,
							},

				form_sub: {
								"nurse_head_job_number":  '', // 护士长工号
										"nurse_head_name":  '', // 护士长姓名
										"position":  '', // 职位
									    "user_id": 0,
					"head_nurse_id": 0 // ID
				},
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["head_nurse_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?head_nurse_id=" + form_sub["head_nurse_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/head_nurse/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/head_nurse/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/head_nurse/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/head_nurse/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/head_nurse/view','get');
