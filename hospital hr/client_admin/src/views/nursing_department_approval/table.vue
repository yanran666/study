		...
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item>
						...
						<el-button v-if="$check_option('/nursing_department_approval/table','export_db')" @click="export_db('dataTable')" style="margin-right: 155px;">导出</el-button>
						<router-link v-if="user_group == '管理员' || $check_action('/nursing_department_approval/table','add') || $check_action('/nursing_department_approval/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/nursing_department_approval/table','del') || $check_action('/nursing_department_approval/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','date_of_approval_by_dake')" min-width="200">
					...
					<el-table-column prop="nursing_department" @sort-change="$sortChange" label="护理部"
				v-if="user_group == '管理员' || $check_field('get','nursing_department')" min-width="200">
						<template slot-scope="scope">
					{{ get_user_nursing_department(scope.row['nursing_department']) }}
				</template>
					</el-table-column>
					<el-table-column prop="approval_date_of_nursing_department" @sort-change="$sortChange" label="护理部批准日期"
				v-if="user_group == '管理员' || $check_field('get','approval_date_of_nursing_department')" min-width="200">
		                <template slot-scope="scope">
                	{{ $toTime(scope.row["approval_date_of_nursing_department"],"yyyy-MM-dd") }}
                </template>
					</el-table-column>
			...
			<el-table-column label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/nursing_department_approval/table','set') || $check_action('/nursing_department_approval/view','set') || $check_action('/nursing_department_approval/view','get') || $check_action('//table','add') || $check_action('//view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/nursing_department_approval/table','set') || $check_action('/nursing_department_approval/view','set') || $check_action('/nursing_department_approval/view','get')"
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/nursing_department_approval/get_list?like=0",
				url_del: "~/api/nursing_department_approval/del?",
		...
		methods: {
			...
			get_list_before(param){
				...
				if(user_group != "管理员"){
						...
						sqlwhere+= "chief_nurse_of_major_department = " + this.user.user_id + " or ";
																		...
																		if(user_group=="护理部"){
						sqlwhere+= "nursing_department = " + this.user.user_id + " or ";
					}
			...
			export_db(tableid){
				...
				// this.$get("~/api/nursing_department_approval/export_db?",query,(json)=>{
				// 	if(json.result){
