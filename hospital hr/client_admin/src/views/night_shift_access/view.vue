	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="护士工号" prop="nurse_no">
													<el-select v-if="user_group === '管理员' || (form['night_shift_access_id'] && $check_field('set','nurse_no')) || (!form['night_shift_access_id'] && $check_field('add','nurse_no'))" id="nurse_no" v-model="form['nurse_no']" :disabled="disabledObj['nurse_no_isDisabled']">
				...
				<el-form-item label="护士姓名" prop="nurse_name">
							  ...
							  v-if="user_group === '管理员' || (form['night_shift_access_id'] && $check_field('set','nurse_name')) || (!form['night_shift_access_id'] && $check_field('add','nurse_name'))" :disabled="disabledObj['nurse_name_isDisabled']"></el-input>
				...
				<el-form-item label="所属大科" prop="major_family">
							  ...
							  v-if="user_group === '管理员' || (form['night_shift_access_id'] && $check_field('set','major_family')) || (!form['night_shift_access_id'] && $check_field('add','major_family'))" :disabled="disabledObj['major_family_isDisabled']"></el-input>
				...
				<el-form-item label="审批状态" prop="approval_status">
						...
						v-if="user_group === '管理员' || (form['night_shift_access_id'] && $check_field('set','approval_status')) || (!form['night_shift_access_id'] && $check_field('add','approval_status'))">
					...
					<div v-else-if="$check_field('get','approval_status')">{{form['approval_status']}}</div>
								...
								<el-col v-if="user_group === '管理员' || $check_field('get','approved_department') || $check_field('add','approved_department') || $check_field('set','approved_department')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="已审批科室" prop="approved_department">
												<el-input id="approved_department" v-model="form['approved_department']" placeholder="请输入已审批科室"
							  v-if="user_group === '管理员' || (form['night_shift_access_id'] && $check_field('set','approved_department')) || (!form['night_shift_access_id'] && $check_field('add','approved_department'))" :disabled="disabledObj['approved_department_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','approved_department')">{{form['approved_department']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="申请日期" prop="application_date">
								<el-date-picker :disabled="disabledObj['application_date_isDisabled']" v-if="user_group === '管理员' || (form['night_shift_access_id'] && $check_field('set','application_date')) || (!form['night_shift_access_id'] && $check_field('add','application_date'))" id="application_date"
				...
				<el-form-item label="批准日期" prop="approval_date">
								<el-date-picker :disabled="disabledObj['approval_date_isDisabled']" v-if="user_group === '管理员' || (form['night_shift_access_id'] && $check_field('set','approval_date')) || (!form['night_shift_access_id'] && $check_field('add','approval_date'))" id="approval_date"
				...
				<el-form-item label="申请资料" prop="application_materials">
					...
					<div v-else>
						<el-upload v-if="user_group === '管理员' || (form['night_shift_access_id'] && $check_field('set','application_materials')) || (!form['night_shift_access_id'] && $check_field('add','application_materials'))" class="upload-demo" drag
				...
				<el-form-item label="已通过科室" prop="passed_department">
							  ...
							  v-if="user_group === '管理员' || (form['night_shift_access_id'] && $check_field('set','passed_department')) || (!form['night_shift_access_id'] && $check_field('add','passed_department'))" :disabled="disabledObj['passed_department_isDisabled']"></el-input>
				...
				<el-form-item label="审批截止日期" prop="approval_deadline">
								<el-date-picker :disabled="disabledObj['approval_deadline_isDisabled']" v-if="user_group === '管理员' || (form['night_shift_access_id'] && $check_field('set','approval_deadline')) || (!form['night_shift_access_id'] && $check_field('add','approval_deadline'))" id="approval_deadline"
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "night_shift_access_id",
				url_add: "~/api/night_shift_access/add?",
				url_set: "~/api/night_shift_access/set?",
				url_get_obj: "~/api/night_shift_access/get_obj?",
				url_upload: "~/api/night_shift_access/upload?",

				query: {
					"night_shift_access_id": 0,
				},
				...
				disabledObj:{
								"nurse_no_isDisabled": false,
										...
										"approved_department_isDisabled": false,
		...
		methods: {
          ...
          this.form["approval_deadline"] = null;
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/night_shift_access/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/night_shift_access/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/night_shift_access/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/night_shift_access/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/night_shift_access/view','get');
