			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="状态" prop="state">
					<el-select v-model="form.state" placeholder="请选择">
						...
						<el-col v-if="user_group === '管理员' || $check_field('get','nursing_department_job_number') || $check_field('add','nursing_department_job_number') || $check_field('set','nursing_department_job_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="护理部工号" prop="nursing_department_job_number">
												<el-input id="nursing_department_job_number" v-model="form_sub['nursing_department_job_number']" placeholder="请输入护理部工号"
							  v-if="user_group === '管理员' || (form_sub['nursing_department_id'] && $check_field('set','nursing_department_job_number')) || (!form_sub['nursing_department_id'] && $check_field('add','nursing_department_job_number'))"></el-input>
					<div v-else-if="$check_field('get','nursing_department_job_number')">{{form_sub['nursing_department_job_number']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','name_of_nursing_department') || $check_field('add','name_of_nursing_department') || $check_field('set','name_of_nursing_department')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="护理部姓名" prop="name_of_nursing_department">
												<el-input id="name_of_nursing_department" v-model="form_sub['name_of_nursing_department']" placeholder="请输入护理部姓名"
							  v-if="user_group === '管理员' || (form_sub['nursing_department_id'] && $check_field('set','name_of_nursing_department')) || (!form_sub['nursing_department_id'] && $check_field('add','name_of_nursing_department'))"></el-input>
					<div v-else-if="$check_field('get','name_of_nursing_department')">{{form_sub['name_of_nursing_department']}}</div>
											</el-form-item>
			</el-col>
	...
	export default {
		...
		data() {
			return {
				...
				group_table: "nursing_department",
				is_password: true,
				...
				disabledObj:{
								"nursing_department_job_number_isDisabled": false,
										"name_of_nursing_department_isDisabled": false,
							},

				form_sub: {
								"nursing_department_job_number":  '', // 护理部工号
										"name_of_nursing_department":  '', // 护理部姓名
									    "user_id": 0,
					"nursing_department_id": 0 // ID
				},
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["nursing_department_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?nursing_department_id=" + form_sub["nursing_department_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/nursing_department/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/nursing_department/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/nursing_department/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/nursing_department/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/nursing_department/view','get');
